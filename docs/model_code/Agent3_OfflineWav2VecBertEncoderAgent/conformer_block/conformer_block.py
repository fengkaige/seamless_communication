# venv_seamless/lib/python3.11/site-packages/fairseq2/models/conformer/block.py
...
class ConformerBlock(TransformerEncoderLayer):
    ...
    def forward(
        ...
        print("="*20, "ConformerBlock forward", seqs.shape)
    ...
    def _forward_ffn1(self, seqs: Tensor) -> Tensor:
        ...
        seqs = self.ffn1_layer_norm(seqs) # 在normalization.py 下一步执行到 StandardLayerNorm.forward()

        seqs = self.ffn1(seqs) * 0.5 # ffn1 : 在 ffn.py 中定义，下一步执行到 StandardFeedForwardNetwork.forward 函数

        if self.ffn1_dropout is not None:
            seqs = self.ffn1_dropout(seqs) # 无
    ...
    def _forward_self_attn(
        ...
        seqs = self.self_attn_layer_norm(seqs) # 在normalization.py 下一步执行到 StandardLayerNorm.forward()
        ...
        ) # multihead_attention.py 下一步执行到 StandardMultiheadAttention.forward()

        if self.self_attn_dropout is not None:
            seqs = self.self_attn_dropout(seqs) # 无
    ...
    def _forward_conv(
        ...
        seqs = self.conv_layer_norm(seqs)  # 在normalization.py 下一步执行到 StandardLayerNorm.forward()

        seqs = self.conv(seqs, padding_mask) # 在convolution.py 下一步执行到 ConformerConvolution.forward()

        if self.conv_dropout is not None:
            seqs = self.conv_dropout(seqs) # 无
    ...
    def _forward_ffn2(self, seqs: Tensor) -> Tensor:
        ...
        seqs = self.ffn2_layer_norm(seqs) # 在normalization.py 下一步执行到 StandardLayerNorm.forward()

        seqs = self.ffn2(seqs) * 0.5 # ffn2 : 在 ffn.py 中定义，下一步执行到 StandardFeedForwardNetwork.forward 函数

        if self.ffn2_dropout is not None:
            seqs = self.ffn2_dropout(seqs) # 无
